...
import {handlerQuitUpdate,handlerQuitShow} from '../../http/quit'
...
export default defineComponent({
     setup(){
        ...
        let handlerUpdateSubmit = () => {
          handlerQuitUpdate(quitForm.value.qid as string, quitForm).then(res => {
      ...
      let initQuitDate = () =>{
          let qid = route.query.qid as string;
          handlerQuitShow(qid).then(res=>{
              
              if(res.data.code === SUCCESS) {
                quitForm.value = res.data.data as QuitForm;
